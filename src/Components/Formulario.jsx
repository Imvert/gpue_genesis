...
export default function Formulario() {
  ...
  useEffect(() => {
    async function misionGet() {
      fetch("http://localhost:3000/mision")
        .then(async (result) => {
    ...
    async function grupoPequenoTipoGet() {
      fetch("http://localhost:3000/gp/tipo")
        .then(async (result) => {
          const response = await result.json();
          if (response.code === 1) {
  ...
  const distritoGet = (event) => {
    fetch(`http://localhost:3000/distrito/${event.target.value}`)
      .then(async (result) => {
        const response = await result.json();
        if (response.code === 1) {
  ...
  const iglesiaGet = (event) => {
    fetch(`http://localhost:3000/iglesia/${event.target.value}`)
      .then(async (result) => {
        const response = await result.json();
        if (response.code === 1) {
  ...
  const onSubmit = (event) => {
    console.log(event);
    debugger;
    config.body = JSON.stringify(event);
    fetch("http://localhost:3000/gp/save", config)
      .then(async (result) => {
        const response = await result.json();
        if (response.code === 1) {
